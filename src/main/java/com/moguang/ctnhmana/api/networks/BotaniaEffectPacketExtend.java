...
public record BotaniaEffectPacketExtend(BotaniaExtendEffectType type, double x, double y, double z, int... args) implements BotaniaPacket {
...
//                            Vec3 receiverVec = VecHelper.fromEntityCenter(e2).add((Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc);
                            ...
                            Vec3 receiverVec = Vec3.atCenterOf(targetBlockpos)
                                    // 加随机偏移（和你之前的粒子逻辑一致）
                                    .add((Math.random() - 0.5) * 0.45, (Math.random() - 0.5) * 0.45, (Math.random() - 0.5) * 0.45);
                        ...
                        case SPARK_MANA_FLOW_REVERSE -> {
                            ...
                            double rc = 0.45;
                            Vec3 receiverVec = VecHelper.fromEntityCenter(e1).add((Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc);
//                            Vec3 receiverVec = VecHelper.fromEntityCenter(e2).add((Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc);
                            BlockPos targetBlockpos=new BlockPos((int) x, (int) y, (int) z);
