...
public record BotaniaEffectPacketExtend(BotaniaExtendEffectType type, double x, double y, double z, int... args)
        implements BotaniaPacket {

    public static final ResourceLocation ID = prefix("effex");
    ...
    public static BotaniaEffectPacketExtend decode(FriendlyByteBuf buf) {
        BotaniaExtendEffectType type = BotaniaExtendEffectType.values()[buf.readByte()];
        ...
        return new BotaniaEffectPacketExtend(type, x, y, z, args);
    }

    public static class Handler {

        public static void handle(BotaniaEffectPacketExtend packet) {
            ...
            mc.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    switch (type) {
                        ...
                        // SparkleParticleData data = SparkleParticleData.sparkle(0.6F + (float) Math.random() * 0.5F, r
                        // / 255F, g / 255F, b / 255F, 5);
                        // world.addParticle(data, pos.getX() + (float) Math.random(), pos.getY() + (float)
                        // Math.random(), pos.getZ() + (float) Math.random(), 0, 0, 0);
                        // }
                        // }
                        ...
                        // x + item.level().random.nextFloat() * item.getBbWidth() * 2.0F - item.getBbWidth() - d0 * d3,
                        // y + item.level().random.nextFloat() * item.getBbHeight() - d1 * d3,
                        // z + item.level().random.nextFloat() * item.getBbWidth() * 2.0F - item.getBbWidth() - d2 * d3,
                        // d0, d1, d2);
                        // }
                        // }
                        case SPARK_NET_INDICATOR -> {
                            ...
                            for (int i = 0; i < iters; i++) {
                                ...
                                world.addAlwaysVisibleParticle(data, true, currentPos.x, currentPos.y, currentPos.z, 0,
                                        0, 0);
                        ...
                        case SPARK_MANA_FLOW -> {
                            ...
                            double rc = 0.45;
                            Vec3 thisVec = VecHelper.fromEntityCenter(e1).add((Math.random() - 0.5) * rc,
                                    (Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc);
                            // Vec3 receiverVec = VecHelper.fromEntityCenter(e2).add((Math.random() - 0.5) * rc,
                            // (Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc);
                            BlockPos targetBlockpos = new BlockPos((int) x, (int) y, (int) z);
                            Vec3 receiverVec = Vec3.atCenterOf(targetBlockpos)
                                    // 加随机偏移（和你之前的粒子逻辑一致）
                                    .add((Math.random() - 0.5) * 0.45, (Math.random() - 0.5) * 0.45,
                            ...
                            world.addAlwaysVisibleParticle(data, thisVec.x, thisVec.y, thisVec.z, motion.x, motion.y,
                                    motion.z);
                        }
                        case SPARK_MANA_FLOW_REVERSE -> {
                            Entity e1 = world.getEntity(args[0]);
                            ...
                            double rc = 0.45;
                            Vec3 receiverVec = VecHelper.fromEntityCenter(e1).add((Math.random() - 0.5) * rc,
                                    (Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc);
                            // Vec3 receiverVec = VecHelper.fromEntityCenter(e2).add((Math.random() - 0.5) * rc,
                            // (Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc);
                            BlockPos targetBlockpos = new BlockPos((int) x, (int) y, (int) z);
                            Vec3 thisVec = Vec3.atCenterOf(targetBlockpos)
                                    // 加随机偏移（和你之前的粒子逻辑一致）
                                    .add((Math.random() - 0.5) * 0.45, (Math.random() - 0.5) * 0.45,
                                            (Math.random() - 0.5) * 0.45);
                            Vec3 motion = receiverVec.subtract(thisVec).scale(0.04F);
                        ...
                        // WispParticleData data = WispParticleData.wisp((float) Math.random() * 0.15F + 0.15F, red,
                        // green, blue);
                        // world.addParticle(data, x, y, z, (float) (Math.random() - 0.5F) * 0.25F, (float)
                        // (Math.random() - 0.5F) * 0.25F, (float) (Math.random() - 0.5F) * 0.25F);
                        // }
                        // }
                        ...
                        // WispParticleData data = WispParticleData.wisp((float) Math.random(), (float) Math.random(),
                        ...
                        // for (int i = 0; i < 50; i++) {
                        // world.addParticle(data, x1 + Math.random() * target.getBbWidth(), y1 + Math.random() *
                        // target.getBbHeight(), z1 + Math.random() * target.getBbWidth(), 0, 0, 0);
                        // }
                        // }
                        // case HALO_CRAFT -> {
                        // Entity target = world.getEntity(args[0]);
                        ...
                        // Vec3 centerVector = VecHelper.fromEntityCenter(target).add(lookVec3.x * 3, 1.3, lookVec3.z *
                        // 3);
                        // float m = 0.1F;
                        // for (int i = 0; i < 4; i++) {
                        // WispParticleData data = WispParticleData.wisp(0.2F + 0.2F * (float) Math.random(), 1F, 0F,
                        // 1F);
                        // target.level().addParticle(data, centerVector.x, centerVector.y, centerVector.z, ((float)
                        // Math.random() - 0.5F) * m, ((float) Math.random() - 0.5F) * m, ((float) Math.random() - 0.5F)
                        // * m);
                        // }
                        // }
                        //
                        ...
                        // for (int i = 0; i < 20; i++) {
                        // for (int j = 0; j < 5; j++) {
                        // WispParticleData data = WispParticleData.wisp(0.35F + (float) Math.random() * 0.1F, 0.25F,
                        // 0.25F, 0.25F);
                        ...
                        // p.getY() + lookDir.y() * i,
                        // p.getZ() + lookDir.z() * i,
                        // 0.2F * (float) (Math.random() - 0.5) * (Math.abs(lookDir.y()) + Math.abs(lookDir.z())) +
                        // -0.01F * (float) Math.random() * lookDir.x(),
                        // 0.2F * (float) (Math.random() - 0.5) * (Math.abs(lookDir.x()) + Math.abs(lookDir.z())) +
                        // -0.01F * (float) Math.random() * lookDir.y(),
                        // 0.2F * (float) (Math.random() - 0.5) * (Math.abs(lookDir.y()) + Math.abs(lookDir.x())) +
                        // -0.01F * (float) Math.random() * lookDir.z());
                        // }
                        // }
                        // }
                        // }
                        ...
                        // GrassSeedsItem.extractR(color), GrassSeedsItem.extractG(color),
                        // GrassSeedsItem.extractB(color));
                        // }
                    }
                }
            });
