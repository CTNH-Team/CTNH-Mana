...
public class ManaHatchStatusProvider extends CapabilityBlockProvider<ManaHatch> {
    ...
    protected @Nullable ManaHatch getCapability(Level level, BlockPos pos, @Nullable Direction side) {
        if (ManaHatch.getMachine(level, pos) instanceof ManaHatch hatch)
            return (ManaHatch) ManaHatch.getMachine(level, pos);
        return null;
    }
    ...
    protected void addTooltip(CompoundTag capData, ITooltip tooltip, Player player, BlockAccessor block,
        ...
        tooltip.add(
                helper.progress(
                        ...
                        Component.translatable("ctnhmana.jade.manahatch.manaprogress",
                                DisplayHelper.dfCommas.format(mana), DisplayHelper.dfCommas.format(maxmana)),
        ...
        if (maxBTMana >= 1) {
            ...
            tooltip.add(
                    helper.progress(
                            progress_bt_mana,
                            Component.translatable("ctnhmana.jade.manahatch.btmanaprogress",
                                    DisplayHelper.dfCommas.format(bt_mana), DisplayHelper.dfCommas.format(maxBTMana)),
        ...
        if (max_lp > 0) {
            ...
            tooltip.add(
                    helper.progress(
                            progress_bm_mana,
                            Component.translatable("ctnhmana.jade.manahatch.bmmanaprogress",
