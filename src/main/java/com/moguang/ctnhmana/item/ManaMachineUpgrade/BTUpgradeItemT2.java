...
public class BTUpgradeItemT2 extends ManaMachineUpgradeItem {
    ...
    public BaseManaMachine.MachineMetric calculateUpgrade(BaseManaMachine.MachineMetric metric, GTRecipe recipe, BaseManaMachine machine)
    {
        ...
        metric.speed+=Math.min(0.4,true_parallel*0.02);
    ...
    public BaseManaMachine.MachineMetric calculateNormalUpgrade(BaseManaMachine.MachineMetric metric,BaseManaMachine machine)
    {
        ...
        metric.parallel+=Math.min(32,(hatch.getBTMana()/50000+hatch.getmaxBTMana()/200000));
    ...
    @CN(
            {
                    "偏向于植物魔法与魔力的升级",
                    "机器获得：",
                    "魔力凝聚仓每存有5W魔力，每存有20W魔力存储上限，就获得一点并行（最高32)",
                    "运行时的每一并行提供§a+2%§r机器工作速度（最高40%）",
