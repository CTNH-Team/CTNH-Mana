...
public class Quasar_Eye extends WorkableElectricMultiblockMachine implements ITieredMachine {
 ...
 private static int[] based_output={0,67108864,134217728,268435456};
    ...
    public void addDisplayText(List<Component> textList) {
        ...
        textList.add(textList.size(), INFO_MANA_PRODUCTION.translate(String.format("%.2f", energy_caculate(rune_energy, energy_tier))));
        textList.add(textList.size(), INFO_RUNE_CONSUMPTION.translate(String.format("%.2f", (rune_energy / 50) * Math.log(rune_energy / 50 + 1))));
        textList.add(textList.size(), INFO_QUASAR_PARALLEL.translate(String.format("%.2f", energy_caculate(rune_energy, energy_tier) * 5)));
        textList.add(textList.size(), INFO_CONSUMPTION_PARALLEL.translate(String.format("%.2f", (1 - 0.05 * Math.max((rune_energy - 50) / 50, 10)))));
